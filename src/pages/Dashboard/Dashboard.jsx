            ...
            <>
              <div className="board-ctn">
                <Link className="board-item" to={"/"}>
                  <p className="number">
                    {formatPrice(orders
                      .filter(order => order.order.sellerId === currentUser._id && order.order.isFinished)
                ...
                <div className='under-board'>
                  ...
                  <Link className="board-item" to={"/work/orders"}>
                    <p className="number">{orders.filter(order => order.order.sellerId === currentUser._id).length}</p>
                    <p className="legend">Commandes</p>
                  </Link>
                </div>
                <Link className="me-recap board-item" to={"/me"}>
                  <ProfilePicture photo={currentUser.img} badge={currentUser.sub ? currentUser.sub : null} />
                  <p className="legend">
                    {
                      currentUser.isSeller ?
                        `${currentUser.name} ${currentUser.lastname.charAt(0)}.` : currentUser.name
                    }
                  </p>
                </Link>
              </div>
              <div className="section">
                ...
                {
                  orders.filter(order => order.order.isFinished !== true && order.order.acceptedBySeller !== false).map(({ order, user, gig, seller }) => (
